<template>
	<div>
		<div class="container clearfix" v-if="!categoryProducts.length">
			<h1 class="text-danger text-center">Opps. No Product Found.</h1>
		</div>
		<div class="container clearfix" v-if="categoryProducts.length">

					<div class="fancy-title title-border topmargin-sm mb-4 title-center">
						<h4 class="text-primary">New Arrivals ( {{categoryName}} )</h4>
					</div>

					<div class="row grid-6">
						<div class="col-lg-2 col-md-3 col-6 px-2" v-for="product in categoryProducts">
							<div class="product">
	...
	methods:{
            getCategoryProduct : function(){
                axios.get('/category-product/'+this.$route.params.id).then((response)=>{
                    ...
                    this.categoryName = response.data.categoryProducts[0].category.category_name;
