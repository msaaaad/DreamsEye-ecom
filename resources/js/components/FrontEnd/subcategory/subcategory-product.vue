<template>
	<div>
		<div class="container clearfix" v-if="!subCategoryProducts.length">
		...
		<div class="container clearfix" v-if="subCategoryProducts.length">
					...
					<div class="fancy-title title-border topmargin-sm mb-4 title-center">
						<h4 class="text-primary">New Arrivals ( {{subCategoryName}} )</h4>
					</div>

					<div class="row grid-6">
						<div class="col-lg-2 col-md-3 col-6 px-2" v-for="product in subCategoryProducts">
							<div class="product">
...
<script>
	...
	methods:{
            getCategoryProduct : function(){
                axios.get('/sub-category-product/'+this.$route.params.id).then((response)=>{
                    this.subCategoryProducts = response.data.subCategoryProducts;
                    this.subCategoryName = response.data.subCategoryProducts[0].subcategory.subcategory_name;
                }).catch((error)=>{
